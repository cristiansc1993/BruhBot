...
{
    var x1 = ( me.x * 5 ) - centerX + 50;
    var x2 = ( me.x * 5 ) + centerX - 50;
    var y1 = ( me.y * 5 ) - centerY + 50;
    var y2 = ( me.y * 5 ) + centerY - 50;
...
function normalize ( p, len )
{
    ...
    var nx = Math.cos( angle ) * len*2;
    var ny = Math.sin( angle ) * len*2;
    if ( !isInsideScreen( p.obj ) )
    {
        return 'left:' + parseInt( centerX + nx ) + 'px;top:' + parseInt( centerY + ny ) + 'px;';
    } else
    {
        return 'left:' + parseInt( centerX - ( me.x - p.obj.x ) * 3 ) + 'px;top:' + parseInt( centerY - ( me.y - p.obj.y ) * 3 ) + 'px;';
    }
}
...
for ( i in img )
{
    ...
    window[ "image" + i ].style.width = "10px";
